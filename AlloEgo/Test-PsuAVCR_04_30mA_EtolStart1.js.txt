...
var starts = [7, 7, 7, 7, 3, 9, 15, 11 ]; // seznam startu, jak jdou za sebou 13, 2, 5, 6, 3   ,6, 5, 10, 2
...
function run() {
	...
	/* nechci nic delat kdyz clovek vejde do cile
  if(preference.get("Aim"+getaim()).entered()){
    ...
    mark.get("Start"+starts[misto]).setVisible(true); // zase zobrazi start
...
function timerTask(name) {	
	...
	if(name == "timelimit"+misto){ // konec zvuku, dalsi faze
		preference.get("Aim"+getaim()).setVisible(true);
